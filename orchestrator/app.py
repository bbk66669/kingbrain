...
# 尝试复用 orchestrator.api 里的 orchestrator 单例
try:
    from orchestrator.api import orchestrator
except Exception:
    from api import orchestrator  # 本地兜底（镜像/路径差异时）

app = FastAPI()


def _mode() -> str:
    """根据 orchestrator 判定或环境变量回退判定运行模式"""
    try:
...
async def _run_phase(req: Request, phase: str):
    """统一的五阶段同步落盘入口：ACK / PLAN / BORROW / DIFF / CR"""
    body: Dict[str, Any] = {}
    try:
    ...
    return JSONResponse(
        payload,
        status_code=(200 if not res.error else 400),
...
# —— 异步（Temporal）接口：惰性导入，避免模块级导入失败拖垮服务 ——
@app.post("/kb-api/submit")
async def submit(payload: Dict[str, Any]):
    try:
        # 惰性导入，只有调用该接口时才需要 temporalio 依赖
        from services.composer.temporal_client import submit_async as submit_wf
    except Exception as e:
        raise HTTPException(503, f"temporal client unavailable: {e}")
    ...
    ctx = {
        ...
        "agents_file": os.path.join(
            os.getenv("REPO_ROOT", "/workspace"), ".collab/agents.yaml"
        ),
        "workflow_id": None,
        "run_id": None,
    }
...
async def get_run(wid: str):
    try:
        # 同样惰性导入
        from services.composer.temporal_client import describe_async as describe_wf
    except Exception as e:
        raise HTTPException(503, f"temporal client unavailable: {e}")
